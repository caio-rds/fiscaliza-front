...
export default function MyProfile() {
  ...
  return (
    <Box
      className={"flexRow"}
      columnGap={"100px"}
      sx={{ height: "100%", width: "100%", paddingTop: "50px" }}
    ...
    >
        ...
        <Box className={"flexColumn"} rowGap={"10px"}>
          ...
          {userEditing ? (
            <Box className={"flexRow"} columnGap={"6px"} rowGap={"6px"}>
              ...
              <Button variant="contained" onClick={() => setuserEditing(false)}>
                Cancelar
              </Button>
            </Box>
          ) : (
            <Button variant="contained" onClick={() => setuserEditing(true)}>
              Editar
            </Button>
          )}
        </Box>
              ...
              {userPayload.address === null ? (
                ...
                <>
                  ...
                  <Typography
                    ...
                    textAlign="left"
                  >
                    {userPayload.address.district}, {userPayload.address.city} -{" "}
                    {userPayload.address.state}
                  </Typography>
                </>
              )}
            </Stack>
          ...
          <Box sx={{ p: 2 }}>
            ...
            >
              ...
              <Typography gutterBottom variant="body2" component="div">
                {new Date(userPayload.createdAt).getDate() +
                  "/" +
                  new Date(userPayload.createdAt).getMonth() +
                  "/" +
                  new Date(userPayload.createdAt).getFullYear()}
              </Typography>
            </Stack>
          </Box>
        </Card>
      ...
      <Modal open={addressEditing} onClose={() => setAddressEditing(false)}>
        <Box sx={style}>
          ...
          <TextField
            ...
            name="street"
            value={
              userPayload.address !== null ? userPayload.address.street : ""
            }
            //onChange={handleChange}
            variant="outlined"
            fullWidth
          ...
          <TextField
            ...
            name="number"
            value={
              userPayload.address !== null ? userPayload.address.number : ""
            }
            //onChange={handleChange}
            variant="outlined"
            fullWidth
          ...
          <TextField
            ...
            name="district"
            value={
              userPayload.address !== null ? userPayload.address.district : ""
            }
            //onChange={handleChange}
            variant="outlined"
            fullWidth
          ...
          <TextField
            ...
            name="state"
            value={
              userPayload.address !== null ? userPayload.address.state : ""
            }
            //onChange={handleChange}
            variant="outlined"
            fullWidth
      ...
      <Modal open={mapShow} onClose={() => setMapShow(false)}>
        <Box sx={style}>
          {userPayload.address !== null ? (
            <MapUser
              lat={parseFloat(userPayload.address.lat)}
              lon={parseFloat(userPayload.address.lon)}
            />
          ) : (
            <Typography variant="h6" gutterBottom>
              Nenhum endereço cadastrado
